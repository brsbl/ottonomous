# ID generation utilities for spec-system
...
# Check if an ID is unique across all specs in the repository
# Usage: validate_unique "my-feature-name-a4b2" "/path/to/repo"
# Returns: 0 if unique, 1 if collision exists
validate_unique() {
    local id="$1"
    local repo_root="${2:-.}"
    local specs_pattern="${repo_root}/**/.kit/specs/*.md"

    # Use find for compatibility (globstar not always available)
    local matching_files
    matching_files=$(find "$repo_root" -path "*/.kit/specs/*.md" -type f 2>/dev/null)

    if [ -z "$matching_files" ]; then
        # No spec files exist, ID is unique
        return 0
    fi

    # Check each spec file for the ID in frontmatter
