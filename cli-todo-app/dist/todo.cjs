...
function formatId(id) {
  return source_default.cyan(`#${id}`);
}
function formatSuccess(msg) {
  return source_default.green("\u2713") + " " + msg;
}
function formatError(msg) {
  return source_default.red("\u2717") + " " + msg;
}
function formatTodo(todo) {
  const id = formatId(todo.id);
  if (todo.done) {
    const checkbox = source_default.green("[\u2713]");
    const text = source_default.strikethrough(todo.text);
    const time = todo.completedAt ? source_default.dim(`(completed ${relativeTime(todo.completedAt)})`) : source_default.dim("(completed)");
    return `  ${id} ${checkbox} ${text} ${time}`;
  } else {
    const checkbox = source_default.yellow("[ ]");
    const time = source_default.dim(`(${relativeTime(todo.createdAt)})`);
